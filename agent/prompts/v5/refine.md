...
The validation gate admits `well_formed` >=35%, `near_miss` >=15%, and
`byte_hostile` from 15% to 45% (with a seven-point smoke-sample margin).
Aim above the floors without raising one class by dropping another below its
floor.
