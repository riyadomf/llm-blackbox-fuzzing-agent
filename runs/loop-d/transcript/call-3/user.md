...
- The input-class mixture. These are **simultaneous minimums, all of which must
  hold at once**, not a budget to move between classes:

  | class | meaning | minimum share |
  |---|---|---|
  | `well_formed` | parses successfully | **>=40%** |
  | `near_miss` | XML-shaped but structurally wrong | **>=20%** |
  | `byte_hostile` | control bytes, truncation, bad encoding, BOMs | **>=20%** |

  Raising one of these by dropping another below its minimum scores worse than
  leaving both alone.
...
The module failed to load or draw examples:

NameError: name 'st' is not defined
  File "<frozen importlib._bootstrap_external>", line 995, in exec_module
  File "<frozen importlib._bootstrap>", line 488, in _call_with_frames_removed
  File "/home/omar/Documents/codes/projects/agent-fuzzing/fuzzer/strategies/gen_iter0.py", line 9, in <module>
    @st.composite
     ^^
NameError: name 'st' is not defined

It must define NAME, DESCRIPTION and documents() -> SearchStrategy[str], importing only hypothesis and the standard library. Fix and resend.