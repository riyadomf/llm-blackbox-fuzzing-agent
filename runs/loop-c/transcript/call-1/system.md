...
6. The strategy must be deterministic given Hypothesis's seed: no `random`,
   no clocks, no I/O.
7. Your generator will be measured on a mixture of THREE input classes, not on
   a single acceptance figure. Producing only well-formed documents scores
   badly however plausible they look. The classes are defined in the task.
