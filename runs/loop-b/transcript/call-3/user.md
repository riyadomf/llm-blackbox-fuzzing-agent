...
```
byte_hostile   share=  6.0%  acceptance= 33.3%  n=30
near_miss      share= 23.0%  acceptance= 24.3%  n=115
well_formed    share= 71.0%  acceptance= 62.3%  n=355
```
...
Overall acceptance: **51.8%** (target band 40-70%)
...
new diagnostic templates this iteration : 1
  + Expected > but got '%s' instead for element <%s/> on line %d.
...
```
Deepest document generated: 669
...
```
    15.1 ms  len=31     depth=0   parsed_clean
     8.7 ms  len=6163   depth=669 parsed_clean
     8.6 ms  len=438    depth=23  parsed_clean
```
