...
- **Acceptance rate: 40.6%** (target band: 40-70%)
- Crashes: 0
...
Reached:
```
  252  g_element_paired
  232  g_element_selfclose
  209  g_chardata
  162  x_name_nonascii
  121  g_attribute_dquot
   73  x_doctype
   72  g_comment
   67  g_attribute_squot
   54  g_pi
   48  x_multiple_roots
   45  g_prolog
   44  x_rootless
   41  x_attribute_unquoted
   35  x_control_char
   34  g_nesting_deep
   32  g_charref_dec
   31  g_charref_hex
   18  x_entity_unsupported
   15  x_mismatched_tags
   13  g_cdata
    7  x_unterminated
    5  x_empty_elem_name
```
...
```
Deepest document generated: 87

### Distinct parser diagnostics reached: 73

```
   53  <%s> cannot be a second root node after <%s> on line %d.
   35  Bad control character 0x%x not allowed by XML standard.
   35  Entity '%s' not supported under parent <%s> on line %d.
   34  Missing close tag </%s> under parent <%s> on line %d.
   19  Mismatched close tag </%s> under parent <%s> on line %d.
   17  Early EOF in comment node on line %d.
    8  Bad control character 0x%x under parent <%s> on line %d not allowed by XML standard.
    5  XML does not start with '%s' (saw '%s').
    4  <%s--> cannot be a second root node after <%s> on line %d.
    4  <%s?> cannot be a second root node after <%s> on line %d.
    4  Early EOF in processing instruction node on line %d.
    4  Missing value for attribute '%s' in element Ii06 on line %d.
    3  Character entity '%s' not terminated under parent <%s> on line %d.
    3  Duplicate attribute '%s' in element :v052OQ on line %d.
    3  Duplicate attribute '%s' in element _S on line %d.
    3  Duplicate attribute '%s' in element f.fep on line %d.
    3  Duplicate attribute '%s' in element g on line %d.
    3  Duplicate attribute '%s' in element gDSGB on line %d.
    3  Duplicate attribute '%s' in element nM on line %d.
    3  Duplicate attribute '%s' in element p on line %d.
```
...
```
    13.1 ms  len=16     depth=0   parsed_clean
    12.1 ms  len=17     depth=0   parsed_clean
     8.3 ms  len=11     depth=0   rejected
```
