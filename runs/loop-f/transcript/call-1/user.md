...
- The input-class mixture. **This is checked mechanically before your strategy
  ...
  A weighting that measures inside the bounds is roughly 9 well-formed entries
  to 5 near-miss to 6 byte-hostile:
  ...
  The measured shares will not match the entry counts exactly, because a
  generator aimed at one class sometimes produces a document that classifies as
  another. Start near this ratio rather than at an even split, which measures
  well outside the bounds.

  `near_miss` is the class that goes wrong most often, and it is worth being
  careful about. The parser is much more tolerant than the grammar, so most ways
  of being malformed are simply accepted and counted as `well_formed`. Build
  near misses only from what it actually rejects:

  - a close tag whose name differs from its open tag, `<a></b>`
  - a named entity outside `amp`, `lt`, `gt`, `quot`, such as `&apos;`
  ...
  Unquoted attribute values, a missing root element, an empty element name and
  an xml declaration with no space are all *accepted*, so they belong in
  `well_formed`, not here.
